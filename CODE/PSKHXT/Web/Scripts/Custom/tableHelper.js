var tableHelper = {
    m_pagerow: 0,
    //初始化Table
    InitTable: function (tb, actionUrl, searchParams, tbCols) {
        $(tb).bootstrapTable({
            ...
            onPageChange: function (number, size) {
                tableHelper.m_pagerow = (number - 1) * size;
            }
        });
    },

    //表格行多选删除
    MultDelete: function (table, actionUrl) {
    ...
    //重置密码
    MultReset: function (table, actionUrl) {
        var arrselections = $(table).bootstrapTable('getSelections');
        if (arrselections.length <= 0) {
            toastr.warning('请选择有效数据');
            return;
